...
import {
	...
	createUpi as createUpiService,
	getUpiById,
	updateUpi,
	getUpis as getUpisService,
	getDenominations as getDenominationsService,
	updateDenominations as updateDenominationsService,
...
export const getWallet = async (req, res, next) => {
	...
	try {
		...
		return res
			.status(200)
			.json(
				new ApiResponse(200, "Wallet fetched successfully", response)
			);
	} catch (error) {
		next(error);
	}
	...
	try {
		...
		return res
			...
			.json(
				new ApiResponse(
					upi ? 200 : 201,
					`Upi ${upi ? "updated" : "created"} successfully`
				)
			);
	} catch (error) {
		next(error);
	...
	try {
		...
		return res
			...
			.json(
				new ApiResponse(
					200,
					"Denominations fetched successfully",
					denominations
				)
			);
	} catch (error) {
	...
	try {
		...
		return res
			...
			.json(
				new ApiResponse(
					200,
					"Denominations updated successfully",
					denominations
				)
			);
	} catch (error) {
