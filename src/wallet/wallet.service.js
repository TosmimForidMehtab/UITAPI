...
export const updateWallet = async (userId, amount) => {
    ...
    return await Wallet.findOneAndUpdate({user: userId}, {$inc: {balance: amount}}, {new: true});
};
...
export const updateUpi = async (userId, upiId) => {
    const upi = await Upi.findOneAndUpdate({user: userId}, {$set: {upiId}}, {new: true});
    return upi;
};
...
export const updateDenominations = async (id, amount) => {
    const denominations = await Denomination.findByIdAndUpdate(id, {$set: {amount}}, {new: true});
    return denominations;
}
